Building the XAX installer

Note: all pathes are relative to the git repository folder (xax-private)
...
Test X11 installer: java -jar xax-client-<version>.jar
Test console installer: java -jar xax-client-<version>.jar -console
...
Copy the content of Java JRE folder (C:\Program Files\Java\jre<version>) into the jre folder under your xax-private git repository preserving the folder structure
...
Test the installer using Java: java -jar xax-client-<version>.jar
Test the windows executable: xax-client-<version>.exe
...
xax.jar is now self executing, the full classpath is embedded into the jar file manifest automatically by jar.sh
