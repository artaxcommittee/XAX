...
jar/exe/dmg package, or run "java -jar xax-client.jar" to start the
installer. See https://bitbucket.org/JeanLucPicard/xax/issues/283 for more
details about the IzPack installer. After installation, use the shortcuts or
desktop icons to start the XAX server.


Using the xax-client.zip package:

Unpack the xax-client.zip package and open a shell in the resulting xax
...
the xax_db blockchain database. The start.sh script can be run from any
directory, it starts the java process in the background and uses ~/.xax for
configuration files, logs, and the xax_db database. Unlike run.sh, start.sh
...
message "XAX server 1.x.x started successfully" in the console log. If run in
...
where the XAX UI should now be available.
...
to the XAX installation directory, as the use of special characters may result
...
conf/xax-default.properties file. All possible settings are listed, with
detailed explanation. If you decide to change any setting, do not edit
xax-default.properties directly, but create a new conf/xax.properties file
and only add to it the properties that need to be different from the default
values. You do not need to delete the defaults from xax-default.properties, the
settings in xax.properties override those in xax-default.properties. This way,
when upgrading the software, you can safely overwrite xax-default.properties
with the updated file from the new package, while your customizations remain
safe in the xax.properties file.


How to contribute?

There are many ways to contribute to XAX. Here are some examples:
...
The XAX software is a client-server application. It consists of a java server
...
requests which other XAX client applications could use.

The blockchain is stored on disk using the H2 embedded database, inside the
xax_db directory. When upgrading, you should not delete the old xax_db
...
xax_db, except that it will take some extra time to download the blockchain
from scratch.

The default XAX client does not store any wallet-type file on disk. Unlike
...
which you see in the console window, but also to a file xax.log, which gets
overwritten at restart. In case of an error, the xax.log file may contain
...
xax.jar file using the enclosed jar.sh script, and then xax.jar should be
