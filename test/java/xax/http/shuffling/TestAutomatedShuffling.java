...
import xax.ShufflingTransaction;
import xax.Tester;
import xax.Transaction;
import xax.crypto.AnonymouslyEncryptedData;
...
import static xax.http.shuffling.ShufflingUtil.ALICE_RECIPIENT;
import static xax.http.shuffling.ShufflingUtil.BOB_RECIPIENT;
import static xax.http.shuffling.ShufflingUtil.CHUCK_RECIPIENT;
import static xax.http.shuffling.ShufflingUtil.DAVE_RECIPIENT;
import static xax.http.shuffling.ShufflingUtil.broadcast;
import static xax.http.shuffling.ShufflingUtil.cancel;
import static xax.http.shuffling.ShufflingUtil.create;
import static xax.http.shuffling.ShufflingUtil.createAssetShuffling;
import static xax.http.shuffling.ShufflingUtil.createCurrencyShuffling;
import static xax.http.shuffling.ShufflingUtil.defaultHoldingShufflingAmount;
import static xax.http.shuffling.ShufflingUtil.defaultShufflingAmount;
import static xax.http.shuffling.ShufflingUtil.getShuffling;
import static xax.http.shuffling.ShufflingUtil.getShufflingParticipants;
import static xax.http.shuffling.ShufflingUtil.process;
import static xax.http.shuffling.ShufflingUtil.register;
import static xax.http.shuffling.ShufflingUtil.shufflingAsset;
import static xax.http.shuffling.ShufflingUtil.shufflingCurrency;
import static xax.http.shuffling.ShufflingUtil.startShuffler;
import static xax.http.shuffling.ShufflingUtil.stopShuffler;
import static xax.http.shuffling.ShufflingUtil.verify;
...
public class TestAutomatedShuffling extends BlockchainTest {
    ...
    public void successfulShuffling() {
        ...
        Assert.assertEquals(-Constants.ONE_XAX, ALICE.getBalanceDiff());
        Assert.assertEquals(-(defaultShufflingAmount + 11 * Constants.ONE_XAX), ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-Constants.ONE_XAX, BOB.getBalanceDiff());
        Assert.assertEquals(-(defaultShufflingAmount + Constants.ONE_XAX), BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-Constants.ONE_XAX, CHUCK.getBalanceDiff());
        Assert.assertEquals(-(defaultShufflingAmount + Constants.ONE_XAX), CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-Constants.ONE_XAX, DAVE.getBalanceDiff());
        Assert.assertEquals(-(defaultShufflingAmount + Constants.ONE_XAX), DAVE.getUnconfirmedBalanceDiff());
        ...
        Assert.assertEquals(-(defaultShufflingAmount + 12 * Constants.ONE_XAX), ALICE.getBalanceDiff());
        Assert.assertEquals(-(defaultShufflingAmount + 12 * Constants.ONE_XAX), ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(defaultShufflingAmount + 12 * Constants.ONE_XAX), BOB.getBalanceDiff());
        Assert.assertEquals(-(defaultShufflingAmount + 12 * Constants.ONE_XAX), BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(defaultShufflingAmount + 12 * Constants.ONE_XAX), CHUCK.getBalanceDiff());
        Assert.assertEquals(-(defaultShufflingAmount + 12 * Constants.ONE_XAX), CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(defaultShufflingAmount + 12 * Constants.ONE_XAX), DAVE.getBalanceDiff());
        Assert.assertEquals(-(defaultShufflingAmount + 12 * Constants.ONE_XAX), DAVE.getUnconfirmedBalanceDiff());
        ...
        Assert.assertEquals(48 * Constants.ONE_XAX, FORGY.getBalanceDiff());
        Assert.assertEquals(48 * Constants.ONE_XAX, FORGY.getUnconfirmedBalanceDiff());
    ...
    public void successfulAssetShuffling() {
        ...
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + 12 * Constants.ONE_XAX), ALICE.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + 12 * Constants.ONE_XAX), ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + 12 * Constants.ONE_XAX), BOB.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + 12 * Constants.ONE_XAX), BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + 12 * Constants.ONE_XAX), CHUCK.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + 12 * Constants.ONE_XAX), CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + 12 * Constants.ONE_XAX), DAVE.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + 12 * Constants.ONE_XAX), DAVE.getUnconfirmedBalanceDiff());
    ...
    public void registrationNotFinished() {
        ...
        Assert.assertEquals(-Constants.ONE_XAX, ALICE.getBalanceDiff());
        Assert.assertEquals(-Constants.ONE_XAX, ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-Constants.ONE_XAX, BOB.getBalanceDiff());
        Assert.assertEquals(-Constants.ONE_XAX, BOB.getUnconfirmedBalanceDiff());

        Assert.assertNull(ALICE_RECIPIENT.getAccount());
        Assert.assertNull(BOB_RECIPIENT.getAccount());

        Assert.assertEquals(2 * Constants.ONE_XAX, FORGY.getBalanceDiff());
        Assert.assertEquals(2 * Constants.ONE_XAX, FORGY.getUnconfirmedBalanceDiff());
    ...
    public void processingNotStarted() {
        ...
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + Constants.ONE_XAX), ALICE.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + Constants.ONE_XAX), ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-Constants.ONE_XAX, BOB.getBalanceDiff());
        Assert.assertEquals(-Constants.ONE_XAX, BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-Constants.ONE_XAX, CHUCK.getBalanceDiff());
        Assert.assertEquals(-Constants.ONE_XAX, CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-Constants.ONE_XAX, DAVE.getBalanceDiff());
        Assert.assertEquals(-Constants.ONE_XAX, DAVE.getUnconfirmedBalanceDiff());
        ...
        Assert.assertEquals(4 * Constants.ONE_XAX + Constants.SHUFFLING_DEPOSIT_NQT, FORGY.getBalanceDiff());
        Assert.assertEquals(4 * Constants.ONE_XAX + Constants.SHUFFLING_DEPOSIT_NQT, FORGY.getUnconfirmedBalanceDiff());
    ...
    public void tooManyParticipants() {
        ...
        Assert.assertEquals(3 * Constants.ONE_XAX + Constants.SHUFFLING_DEPOSIT_NQT, FORGY.getBalanceDiff());
        Assert.assertEquals(3 * Constants.ONE_XAX + Constants.SHUFFLING_DEPOSIT_NQT, FORGY.getUnconfirmedBalanceDiff());
    ...
    public void processingNotFinished() {
        ...
        Assert.assertEquals(-11 * Constants.ONE_XAX, ALICE.getBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_XAX, ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_XAX, BOB.getBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_XAX, BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_XAX, CHUCK.getBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_XAX, CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + Constants.ONE_XAX), DAVE.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + Constants.ONE_XAX), DAVE.getUnconfirmedBalanceDiff());
        ...
        Assert.assertEquals(34 * Constants.ONE_XAX + Constants.SHUFFLING_DEPOSIT_NQT, FORGY.getBalanceDiff());
        Assert.assertEquals(34 * Constants.ONE_XAX + Constants.SHUFFLING_DEPOSIT_NQT, FORGY.getUnconfirmedBalanceDiff());
    ...
    public void verifyNotFinished() {
        ...
        Assert.assertEquals(-12 * Constants.ONE_XAX, ALICE.getBalanceDiff());
        Assert.assertEquals(-12 * Constants.ONE_XAX, ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-12 * Constants.ONE_XAX, BOB.getBalanceDiff());
        Assert.assertEquals(-12 * Constants.ONE_XAX, BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + 11 * Constants.ONE_XAX), CHUCK.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + 11 * Constants.ONE_XAX), CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-12 * Constants.ONE_XAX, DAVE.getBalanceDiff());
        Assert.assertEquals(-12 * Constants.ONE_XAX, DAVE.getUnconfirmedBalanceDiff());
        ...
        Assert.assertEquals(47 * Constants.ONE_XAX + Constants.SHUFFLING_DEPOSIT_NQT, FORGY.getBalanceDiff());
        Assert.assertEquals(47 * Constants.ONE_XAX + Constants.SHUFFLING_DEPOSIT_NQT, FORGY.getUnconfirmedBalanceDiff());
    ...
    public void cancelAfterVerifyChuck() {
        ...
        Assert.assertEquals(-22 * Constants.ONE_XAX, ALICE.getBalanceDiff());
        Assert.assertEquals(-22 * Constants.ONE_XAX, ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-22 * Constants.ONE_XAX, BOB.getBalanceDiff());
        Assert.assertEquals(-22 * Constants.ONE_XAX, BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + 21 * Constants.ONE_XAX), CHUCK.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + 21 * Constants.ONE_XAX), CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-12 * Constants.ONE_XAX, DAVE.getBalanceDiff());
        Assert.assertEquals(-12 * Constants.ONE_XAX, DAVE.getUnconfirmedBalanceDiff());
        ...
        Assert.assertEquals(77 * Constants.ONE_XAX + Constants.SHUFFLING_DEPOSIT_NQT, FORGY.getBalanceDiff());
        Assert.assertEquals(77 * Constants.ONE_XAX + Constants.SHUFFLING_DEPOSIT_NQT, FORGY.getUnconfirmedBalanceDiff());
    ...
    public void cancelAfterVerifyChuckInvalidKeysAlice() {
        ...
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + 22 * Constants.ONE_XAX), ALICE.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + 22 * Constants.ONE_XAX), ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-22 * Constants.ONE_XAX, BOB.getBalanceDiff());
        Assert.assertEquals(-22 * Constants.ONE_XAX, BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-21 * Constants.ONE_XAX, CHUCK.getBalanceDiff());
        Assert.assertEquals(-21 * Constants.ONE_XAX, CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-12 * Constants.ONE_XAX, DAVE.getBalanceDiff());
        Assert.assertEquals(-12 * Constants.ONE_XAX, DAVE.getUnconfirmedBalanceDiff());
    ...
    public void badProcessDataAlice() {
        ...
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + 11 * Constants.ONE_XAX), ALICE.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + 11 * Constants.ONE_XAX), ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_XAX, BOB.getBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_XAX, BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-1 * Constants.ONE_XAX, CHUCK.getBalanceDiff());
        Assert.assertEquals(-1 * Constants.ONE_XAX, CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-1 * Constants.ONE_XAX, DAVE.getBalanceDiff());
        Assert.assertEquals(-1 * Constants.ONE_XAX, DAVE.getUnconfirmedBalanceDiff());
        ...
        Assert.assertEquals(24 * Constants.ONE_XAX + Constants.SHUFFLING_DEPOSIT_NQT, FORGY.getBalanceDiff());
        Assert.assertEquals(24 * Constants.ONE_XAX + Constants.SHUFFLING_DEPOSIT_NQT, FORGY.getUnconfirmedBalanceDiff());
    ...
    public void modifiedProcessDataBob() {
        ...
        Assert.assertEquals(-21 * Constants.ONE_XAX, ALICE.getBalanceDiff());
        Assert.assertEquals(-21 * Constants.ONE_XAX, ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + (bobCancelFailed ? 11 : 21) * Constants.ONE_XAX), BOB.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + (bobCancelFailed ? 11 : 21) * Constants.ONE_XAX), BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_XAX, CHUCK.getBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_XAX, CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-1 * Constants.ONE_XAX, DAVE.getBalanceDiff());
        Assert.assertEquals(-1 * Constants.ONE_XAX, DAVE.getUnconfirmedBalanceDiff());
        ...
        Assert.assertEquals((bobCancelFailed ? 44 : 54) * Constants.ONE_XAX + Constants.SHUFFLING_DEPOSIT_NQT, FORGY.getBalanceDiff());
        Assert.assertEquals((bobCancelFailed ? 44 : 54) * Constants.ONE_XAX + Constants.SHUFFLING_DEPOSIT_NQT, FORGY.getUnconfirmedBalanceDiff());
    ...
    public void duplicateProcessDataBob() {
        ...
        Assert.assertEquals(-21 * Constants.ONE_XAX, ALICE.getBalanceDiff());
        Assert.assertEquals(-21 * Constants.ONE_XAX, ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + 21 * Constants.ONE_XAX), BOB.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + 21 * Constants.ONE_XAX), BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_XAX, CHUCK.getBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_XAX, CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-1 * Constants.ONE_XAX, DAVE.getBalanceDiff());
        Assert.assertEquals(-1 * Constants.ONE_XAX, DAVE.getUnconfirmedBalanceDiff());
        ...
        Assert.assertEquals(54 * Constants.ONE_XAX + Constants.SHUFFLING_DEPOSIT_NQT, FORGY.getBalanceDiff());
        Assert.assertEquals(54 * Constants.ONE_XAX + Constants.SHUFFLING_DEPOSIT_NQT, FORGY.getUnconfirmedBalanceDiff());
    ...
    public void modifiedProcessDataChuck() {
        ...
        Assert.assertEquals(-21 * Constants.ONE_XAX, ALICE.getBalanceDiff());
        Assert.assertEquals(-21 * Constants.ONE_XAX, ALICE.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-21 * Constants.ONE_XAX, BOB.getBalanceDiff());
        Assert.assertEquals(-21 * Constants.ONE_XAX, BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + (chuckCancelFailed ? 11 : 21) * Constants.ONE_XAX), CHUCK.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + (chuckCancelFailed ? 11 : 21) * Constants.ONE_XAX), CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-12 * Constants.ONE_XAX, DAVE.getBalanceDiff());
        Assert.assertEquals(-12 * Constants.ONE_XAX, DAVE.getUnconfirmedBalanceDiff());
        ...
        Assert.assertEquals((chuckCancelFailed ? 65 : 75) * Constants.ONE_XAX + Constants.SHUFFLING_DEPOSIT_NQT, FORGY.getBalanceDiff());
        Assert.assertEquals((chuckCancelFailed ? 65 : 75) * Constants.ONE_XAX + Constants.SHUFFLING_DEPOSIT_NQT, FORGY.getUnconfirmedBalanceDiff());
    ...
    public void modifiedRecipientKeysDave() {
        ...
        Assert.assertEquals(-65 * Constants.ONE_XAX, ALICE.getBalanceDiff() + BOB.getBalanceDiff() + CHUCK.getBalanceDiff());
    ...
    public void duplicateProcessDataChuck() {
        ...
        Assert.assertEquals(-11 * Constants.ONE_XAX, BOB.getBalanceDiff());
        Assert.assertEquals(-11 * Constants.ONE_XAX, BOB.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + Constants.ONE_XAX), CHUCK.getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + Constants.ONE_XAX), CHUCK.getUnconfirmedBalanceDiff());
        Assert.assertEquals(-1 * Constants.ONE_XAX, DAVE.getBalanceDiff());
        Assert.assertEquals(-1 * Constants.ONE_XAX, DAVE.getUnconfirmedBalanceDiff());
    ...
    public void maxShufflingSize() {
        ...
        for (int i = 0; i < n + 5; i++) {
            generateBlock();
            Block block = XAX.getBlockchain().getLastBlock();
        ...
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + 12 * Constants.ONE_XAX) + 100 * Constants.ONE_XAX, participants[n - 1].getBalanceDiff());
        Assert.assertEquals(-(Constants.SHUFFLING_DEPOSIT_NQT + 12 * Constants.ONE_XAX) + 100 * Constants.ONE_XAX, participants[n - 1].getUnconfirmedBalanceDiff());
