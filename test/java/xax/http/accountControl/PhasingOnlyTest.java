...
package xax.http.accountControl;

import xax.BlockchainTest;
import xax.Constants;
import xax.XAX;
import xax.PhasingParams;
import xax.VoteWeighting.MinBalanceModel;
import xax.VoteWeighting.VotingModel;
import xax.http.APICall;
import xax.http.APICall.Builder;
...
public class PhasingOnlyTest extends BlockchainTest {
    ...
    public void testAccountVoting() throws Exception {
        ...
        Builder builder = new ACTestUtils.Builder("sendMoney", ALICE.getSecretPhrase())
            .recipient(BOB.getId())
            .param("amountNQT", 1 * Constants.ONE_XAX);
    ...
    public void testExtraRestrictions() throws Exception {
        //all transactions must be approved either by BOB or CHUCK, total fees 5 XAX, min duration 4, max duration 100
        setPhasingOnlyControl(VotingModel.ACCOUNT, null, 1L, null, null, new long[] {BOB.getId(), CHUCK.getId()},
                5 * Constants.ONE_XAX, 4, 100);

        Builder builder = new ACTestUtils.Builder("sendMoney", ALICE.getSecretPhrase())
                .recipient(BOB.getId())
                .param("amountNQT", 1 * Constants.ONE_XAX)
                .feeNQT(7 * Constants.ONE_XAX);
        // fee too high
        setTransactionPhasingParams(builder, 20, VotingModel.ACCOUNT, null, 1L, null, null, new long[] {BOB.getId(), CHUCK.getId()});
        ACTestUtils.assertTransactionBlocked(builder);

        // fee at the limit
        builder.feeNQT(5 * Constants.ONE_XAX);
        ...
        builder.param("phasingFinishHeight", XAX.getBlockchain().getHeight() + 200);
        ACTestUtils.assertTransactionBlocked(builder);
        builder.param("phasingFinishHeight", XAX.getBlockchain().getHeight() + 3);
        ACTestUtils.assertTransactionBlocked(builder);
        builder.param("phasingFinishHeight", XAX.getBlockchain().getHeight() + 4);
        ACTestUtils.assertTransactionSuccess(builder);
    ...
    public void testRejectingPendingTransaction() throws Exception {
        ...
        Assert.assertEquals(balanceBeforeTransactionRejection + 1 * Constants.ONE_XAX, 
                ACTestUtils.getAccountBalance(ALICE.getId(), "unconfirmedBalanceNQT"));
    }
    
    @Test
    public void testBalanceVoting() {
        setPhasingOnlyControl(VotingModel.NQT, null, 100 * Constants.ONE_XAX, null, null, null, 0, 0, 0);
        ...
        setTransactionPhasingParams(builder, 20, VotingModel.NQT, null, 100 * Constants.ONE_XAX, null, null, new long[] {DAVE.getId()});
        ACTestUtils.assertTransactionBlocked(builder);
        
        setTransactionPhasingParams(builder, 20, VotingModel.ACCOUNT, null, 1L, null, null, new long[] {BOB.getId(), CHUCK.getId()});
        ACTestUtils.assertTransactionBlocked(builder);
        
        setTransactionPhasingParams(builder, 20, VotingModel.NQT, null, 100 * Constants.ONE_XAX + 1, null, null, null);
        ACTestUtils.assertTransactionBlocked(builder);
        ...
        setTransactionPhasingParams(builder, 20, VotingModel.NQT, null, 100 * Constants.ONE_XAX, null, null, null);
        ACTestUtils.assertTransactionSuccess(builder);
    }
    ...
    private void setTransactionPhasingParams(Builder builder, int finishAfter, VotingModel votingModel, String holdingId, Long quorum,
        ...
        builder.param("phasingFinishHeight", XAX.getBlockchain().getHeight() + finishAfter);
