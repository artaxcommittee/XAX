...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.sendRequest = function (requestType, data, callback, options) {
        ...
        try {
            var xaxFields = [
                ...
                ["priceXAX", "priceNQT"],
                ["refundXAX", "refundNQT"],
                ["discountXAX", "discountNQT"],
                ["phasingQuorumXAX", "phasingQuorum"],
                ["phasingMinBalanceXAX", "phasingMinBalance"],
                ["controlQuorumXAX", "controlQuorum"],
                ["controlMinBalanceXAX", "controlMinBalance"],
                ["controlMaxFeesXAX", "controlMaxFees"],
                ["minBalanceXAX", "minBalance"],
                ["shufflingAmountXAX", "amount"],
                ["monitorAmountXAX", "amount"],
                ["monitorThresholdXAX", "threshold"]
            ];

            for (i = 0; i < xaxFields.length; i++) {
                var xaxField = xaxFields[i][0];
                var nqtField = xaxFields[i][1];
                if (xaxField in data) {
                    data[nqtField] = NRS.convertToNQT(data[xaxField]);
                    delete data[xaxField];
                }
            }
        } catch (err) {
            callback({
            ...
            if (maxFees > 0 && new BigInteger(data.feeNQT).compareTo(new BigInteger(phasingControl.maxFees)) > 0) {
                callback({
                    ...
                    "errorDescription": $.t("error_fee_exceeds_max_account_control_fee", {
                        "maxFee": NRS.convertToXAX(phasingControl.maxFees)
                    })
                });
                return;
            }
        ...
        if (requestType == "getAccountId") {
            ...
            var accountRS = "";
            if (xaxAddress.set(accountId)) {
                accountRS = xaxAddress.toString();
            }
            callback({
                "account": accountId,
                "accountRS": accountRS
    ...
    NRS.processAjaxRequest = function (requestType, data, callback, options) {
        ...
        if (options.remoteNode) {
            url = options.remoteNode.getUrl() + "/xax";
        } else {
