...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.listAccounts = function() {
        ...
        loginAccount.empty();
		if (NRS.getStrItem("savedXAXAccounts") && NRS.getStrItem("savedXAXAccounts") != ""){
			$('#login_account_container').show();
			$('#login_account_container_other').hide();
			var accounts = NRS.getStrItem("savedXAXAccounts").split(";");
    ...
    $("#loginButtons").find(".btn").click(function (e) {
        ...
        if (type == "account") {
            ...
            NRS.scanQRCode(readerId, function(text) {
                var xaxAddress = new XAXAddress();
                if (xaxAddress.set(text)) {
	...
	NRS.removeAccount = function(account) {
		var accounts = NRS.getStrItem("savedXAXAccounts").replace(account+';','');
		if (accounts == '') {
			NRS.removeItem('savedXAXAccounts');
		} else {
			NRS.setStrItem("savedXAXAccounts", accounts);
		}
		NRS.listAccounts();
	};

    function rememberAccount(account) {
        var accountsStr = NRS.getStrItem("savedXAXAccounts");
        if (!accountsStr) {
            NRS.setStrItem("savedXAXAccounts", account + ";");
            return;
        }
        ...
        NRS.setStrItem("savedXAXAccounts", accountsStr + account + ";");
    }
			...
			NRS.sendRequest(accountRequest, requestVariable, function(response, data) {
				...
				NRS.sendRequest("getAccountPublicKey", {
					...
					if (NRS.getStrItem("savedXAXAccounts") && NRS.getStrItem("savedXAXAccounts")!=""){
						...
						accounts = NRS.getStrItem("savedXAXAccounts").split(";");
						$.each(accounts, function(index, account) {
							if (account != ''){
	...
	NRS.initPluginWarning = function() {
		if (NRS.activePlugins) {
			...
			html += "Plugins are not sandboxed or restricted in any way and have full accesss to your client system including your XAX passphrase.";
			html += "</p>";
			...
			html += "you can loose your XAX! In doubt don't run plugins with accounts ";
			html += "used to store larger amounts of XAX now or in the future.";
			html += "</p>";
			html += "</div>";
