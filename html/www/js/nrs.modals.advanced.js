...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.initAdvancedModalFormValues = function($modal) {
		...
		context = {
			...
			inputName: "phasingQuorumXAX",
			addonText: "XAX",
			addonI18n: "xax_unit"
		};
		$elems = NRS.initModalUIElement($modal, '.approve_transaction_amount_xax', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		context['inputName'] = 'phasingMinBalanceXAX';
		context['addonText'] = 'XAX';
		context['addonI18n'] = 'xax_unit';
		$elems = NRS.initModalUIElement($modal, '.approve_min_balance_xax', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
