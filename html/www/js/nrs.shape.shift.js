...
var NRS = (function(NRS, $) {
    ...
    var coinToPair = function (op, coin) {
        return (op == "buy") ? "XAX_" + coin : coin + "_XAX";
    };

    var pairToCoin = function (pair) {
        if (pair.indexOf("XAX_") == 0) {
            return pair.substring("XAX_".length);
        }
        if (pair.indexOf("_XAX") == pair.length - "_XAX".length) {
            return pair.substring(0, pair.indexOf("_XAX"));
        }
    ...
    var renderExchangeTable = function (op) {
        ...
        async.series(tasks, function (err, results) {
            var table = $("#p_shape_shift_" + op + "_xax");
            if (err) {
        ...
        apiCall('recenttx/50', {}, 'GET', function (data) {
            ...
            if (data) {
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (String(transaction.curIn).escapeHTML() != "XAX" && String(transaction.curOut).escapeHTML() != "XAX") {
                        continue;
                    }
                    rows += "<tr>";
    ...
    function renderXAXLimit() {
        apiCall('limit/xax_btc', {}, 'GET', function (data) {
            NRS.logConsole("limit1 " + data.limit);
            if (data.limit) {
                $('#shape_shift_status').html('ok');
                $('#shape_shift_xax_avail').html(String(data.limit).escapeHTML());
            }
        });
    }
        ...
        apiCall('getcoins', {}, 'GET', function (data) {
            ...
            for (var i = 0; i < inputFields.length; i++) {
                ...
                $.each(data, function (code, coin) {
                    if (code != 'XAX' && coin['status'] == 'available') {
    ...
    NRS.getFundAccountLink = function() {
        return "<div class='callout callout-danger'>" +
            ...
            "data-pair='BTC_XAX'>" + $.t("fund_account_message") + "</a>";
    };
    ...
    $("#m_shape_shift_buy_modal").on("show.bs.modal", function (e) {
        ...
        $("#m_shape_shift_buy_title").html($.t("exchange_xax_to_coin_shift", { coin: coin }));
        $("#m_shape_shift_buy_min").val(invoker.data("min"));
        $("#m_shape_shift_buy_min_coin").html("XAX");
        $("#m_shape_shift_buy_max").val(invoker.data("max"));
        $("#m_shape_shift_buy_max_coin").html("XAX");
        $("#m_shape_shift_buy_rate").val(invoker.data("rate"));
        $("#m_shape_shift_buy_rate_text").html("XAX/" + coin);
    ...
    $("#m_send_amount_buy_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_send_amount_buy_title").html($.t("exchange_xax_to_coin_send_amount", { coin: coin }));
        $("#m_send_amount_buy_withdrawal_amount_coin").html(coin);
        $("#m_send_amount_buy_rate_text").html("XAX/" + coin);
    ...
    $('#m_send_amount_buy_withdrawal_amount, #m_send_amount_buy_withdrawal_address').change(function () {
        ...
        apiCall('sendamount', {
            ...
            try {
                ...
                // add 1 XAX fee to make sure the net amount is what requested by shape shift
    ...
    $("#m_shape_shift_sell_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_shape_shift_sell_title").html($.t("exchange_coin_to_xax_shift", { coin: coin }));
        ...
        async.waterfall([
            ...
            function(callback) {
                ...
                $("#m_shape_shift_sell_rate_text").html(coin + "/XAX");
                $("#m_shape_shift_sell_fee").val(data.fee);
                $("#m_shape_shift_sell_fee_coin").html("XAX");
    ...
    $("#m_send_amount_sell_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_send_amount_sell_title").html($.t("exchange_coin_to_xax_send_amount", { coin: coin }));
        $("#m_send_amount_sell_rate_text").html("XAX/" + coin);
        $("#m_send_amount_sell_fee_coin").html("XAX");
        $("#m_send_amount_sell_withdrawal_amount_coin").html("XAX");
