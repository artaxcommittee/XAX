...
public abstract class EntityDbTable<T> extends DerivedDbTable {
    ...
    public void checkAvailable(int height) {
        if (multiversion) {
            int minRollBackHeight = isPersistent() && XAX.getBlockchainProcessor().isScanning() ?
                    Math.max(XAX.getBlockchainProcessor().getInitialScanHeight() - Constants.MAX_ROLLBACK, 0)
                    : XAX.getBlockchainProcessor().getMinRollbackHeight();
    ...
    private boolean doesNotExceed(int height) {
        return XAX.getBlockchain().getHeight() <= height && ! (isPersistent() && XAX.getBlockchainProcessor().isScanning());
    }
