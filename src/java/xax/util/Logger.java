...
/**
 * Handle logging for the XAX node server
 */
public final class Logger {
     ...
     * The xax/conf/logging-default.properties and xax/conf/logging.properties configuration
    ...
    static {
        ...
        System.setProperty("java.util.logging.manager", "xax.util.XAXLogManager");
        if (!(LogManager.getLogManager() instanceof XAXLogManager)) {
            System.setProperty("java.util.logging.manager",
                    (oldManager != null ? oldManager : "java.util.logging.LogManager"));
        }
        if (! Boolean.getBoolean("xax.doNotConfigureLogging")) {
            try {
                Properties loggingProperties = new Properties();
                XAX.loadProperties(loggingProperties, "logging-default.properties", true);
                XAX.loadProperties(loggingProperties, "logging.properties", false);
                XAX.updateLogFileHandler(loggingProperties);
        ...
        log = org.slf4j.LoggerFactory.getLogger(xax.XAX.class);
        enableStackTraces = XAX.getBooleanProperty("xax.enableStackTraces");
        enableLogTraceback = XAX.getBooleanProperty("xax.enableLogTraceback");
    ...
    public static void shutdown() {
        if (LogManager.getLogManager() instanceof XAXLogManager) {
            ((XAXLogManager) LogManager.getLogManager()).xaxShutdown();
        }
    }
