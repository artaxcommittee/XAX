...
/**
 * Java LogManager extension for use with XAX
 */
public class XAXLogManager extends LogManager {

    /** Logging reconfiguration in progress */
    private volatile boolean loggingReconfiguration = false;

    /**
     * Create the XAX log manager
     *
     ...
     * done terminating the XAX processes.
     */
    public XAXLogManager() {
        super();
    }
     ...
     * This allows us to continue to use logging facilities during XAX shutdown.
     */
    @Override
    public void reset() {
    ...
    /**
     * XAX shutdown is now complete, so call LogManager.reset() to terminate
     * the log handlers.
     */
    void xaxShutdown() {
        super.reset();
