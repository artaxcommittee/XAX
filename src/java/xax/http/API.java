...
import static xax.http.JSONResponses.INCORRECT_ADMIN_PASSWORD;
import static xax.http.JSONResponses.NO_PASSWORD_IN_CONFIG;
import static xax.http.JSONResponses.LOCKED_ADMIN_PASSWORD;
...
public final class API {
    ...
    public static final String adminPassword = XAX.getStringProperty("xax.adminPassword", "", true);
    static final boolean disableAdminPassword;
    static final int maxRecords = XAX.getIntProperty("xax.maxAPIRecords");
    static final boolean enableAPIUPnP = XAX.getBooleanProperty("xax.enableAPIUPnP");
    public static final int apiServerIdleTimeout = XAX.getIntProperty("xax.apiServerIdleTimeout");
    public static final boolean apiServerCORS = XAX.getBooleanProperty("xax.apiServerCORS");
    ...
    static {
        List<String> disabled = new ArrayList<>(XAX.getStringListProperty("xax.disabledAPIs"));
        Collections.sort(disabled);
        disabledAPIs = Collections.unmodifiableList(disabled);
        disabled = XAX.getStringListProperty("xax.disabledAPITags");
        Collections.sort(disabled);
        ...
        List<String> allowedBotHostsList = XAX.getStringListProperty("xax.allowedBotHosts");
        ...
        boolean enableAPIServer = XAX.getBooleanProperty("xax.enableAPIServer");
        if (enableAPIServer) {
            final int port = Constants.isTestnet ? TESTNET_API_PORT : XAX.getIntProperty("xax.apiServerPort");
            final int sslPort = Constants.isTestnet ? TESTNET_API_SSLPORT : XAX.getIntProperty("xax.apiServerSSLPort");
            final String host = XAX.getStringProperty("xax.apiServerHost");
            disableAdminPassword = XAX.getBooleanProperty("xax.disableAdminPassword") || ("127.0.0.1".equals(host) && adminPassword.isEmpty());

            apiServer = new Server();
            ServerConnector connector;
            boolean enableSSL = XAX.getBooleanProperty("xax.apiSSL");
            //
            ...
            if (enableSSL) {
                ...
                String keyStorePath = Paths.get(XAX.getUserHomeDir()).resolve(Paths.get(XAX.getStringProperty("xax.keyStorePath"))).toString();
                Logger.logInfoMessage("Using keystore: " + keyStorePath);
                sslContextFactory.setKeyStorePath(keyStorePath);
                sslContextFactory.setKeyStorePassword(XAX.getStringProperty("xax.keyStorePassword", null, true));
                ...
                sslContextFactory.setKeyStoreType(XAX.getStringProperty("xax.keyStoreType"));
                List<String> ciphers = XAX.getStringListProperty("xax.apiSSLCiphers");
                if (!ciphers.isEmpty()) {
            ...
            String apiResourceBase = XAX.getStringProperty("xax.apiResourceBase");
            if (apiResourceBase != null) {
                ...
                apiHandler.setWelcomeFiles(new String[]{XAX.getStringProperty("xax.apiWelcomeFile")});
            }

            String javadocResourceBase = XAX.getStringProperty("xax.javadocResourceBase");
            ...
            ServletHolder servletHolder = apiHandler.addServlet(APIServlet.class, "/xax");
            servletHolder.getRegistration().setMultipartConfig(new MultipartConfigElement(
                    null, Math.max(XAX.getIntProperty("xax.maxUploadFileSize"), Constants.MAX_TAGGED_DATA_DATA_LENGTH), -1L, 0));

            servletHolder = apiHandler.addServlet(APIProxyServlet.class, "/xax-proxy");
            ...
            if (!XAX.getBooleanProperty("xax.enableAPIServerGZIPFilter")) {
                gzipHandler.setExcludedPaths("/xax", "/xax-proxy");
            }
            gzipHandler.setIncludedMethods("GET", "POST");
            gzipHandler.setMinGzipSize(xax.peer.Peers.MIN_COMPRESS_SIZE);
            ...
            if (XAX.getBooleanProperty("xax.apiFrameOptionsSameOrigin")) {
