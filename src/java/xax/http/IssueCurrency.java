...
/**
 * Issue a currency on the XAX blockchain
 * <p>
 * A currency is the basic block of the XAX Monetary System it can be exchanged with XAX, transferred between accounts,
 ...
 * For {@link xax.CurrencyType#RESERVABLE} currency
 * <li>minReservePerUnitNQT - the minimum XAX value per unit to allow the currency to become active
 * For {@link xax.CurrencyType#RESERVABLE} currency
 ...
 * For {@link xax.CurrencyType#MINTABLE} currency
 * <li>maxDifficulty - for mint-able currency, the exponent of the final difficulty.
 * For {@link xax.CurrencyType#MINTABLE} currency
 * <li>algorithm - the hashing {@link xax.crypto.HashFunction algorithm} used for minting.
 * For {@link xax.CurrencyType#MINTABLE} currency
 * </ul>
 * <p>
 * Constraints
 * <ul>
 * <li>A given currency can not be neither {@link xax.CurrencyType#EXCHANGEABLE} nor {@link xax.CurrencyType#CLAIMABLE}.<br>
 * <li>A {@link xax.CurrencyType#RESERVABLE} currency becomes active once the blockchain height reaches the currency issuance height.<br>
 ...
 * When a {@link xax.CurrencyType#RESERVABLE} becomes active, in case it is {@link xax.CurrencyType#CLAIMABLE} the XAX used for
 * reserving the currency are locked until they are claimed back.
 * When a {@link xax.CurrencyType#RESERVABLE} becomes active, in case it is non {@link xax.CurrencyType#CLAIMABLE} the XAX used for
 * reserving the currency are sent to the issuer account as crowd funding.
 * <li>When issuing a {@link xax.CurrencyType#MINTABLE} currency, the number of units per {@link xax.http.CurrencyMint} cannot exceed 0.01% of the
 ...
 * difficulty increases linearly with the number units minted per {@link xax.http.CurrencyMint}<br>
 * </ul>
 *
 * @see xax.CurrencyType
 * @see xax.crypto.HashFunction
 */
