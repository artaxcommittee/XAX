...
import xax.AccountLedger;
import xax.AccountLedger.LedgerEntry;
import xax.Block;
import xax.BlockchainProcessor;
import xax.Db;
import xax.XAX;
import xax.Transaction;
import xax.TransactionProcessor;
 ...
 * has not been received within xax.apiEventTimeout seconds.
 *
 * The maximum number of event users is specified by xax.apiMaxEventUsers.
 */
class EventListener implements Runnable, AsyncListener, TransactionalDb.TransactionCallback {

    /** Maximum event users */
    static final int maxEventUsers = XAX.getIntProperty("xax.apiMaxEventUsers");

    /** Event registration timeout (seconds) */
    static final int eventTimeout = Math.max(XAX.getIntProperty("xax.apiEventTimeout"), 15);

    /** Blockchain processor */
    static final BlockchainProcessor blockchainProcessor = XAX.getBlockchainProcessor();

    /** Transaction processor */
    static final TransactionProcessor transactionProcessor = XAX.getTransactionProcessor();
    ...
    /** XAX event listeners */
    private final List<XAXEventListener> xaxEventListeners = new ArrayList<>();
     ...
     *
     * XAX event listeners will be added for the specified events
     *
     * @param   eventRegistrations      List of XAX event registrations
     ...
     *
     * @param   eventRegistrations      XAX event registrations
     * @throws  EventListenerException  Invalid XAX event
     */
    void addEvents(List<EventRegistration> eventRegistrations) throws EventListenerException {
        ...
        try {
            ...
            for (EventRegistration event : eventRegistrations) {
                ...
                Iterator<XAXEventListener> it = xaxEventListeners.iterator();
                while (it.hasNext()) {
                    XAXEventListener listener = it.next();
                ...
                if (addListener) {
                    XAXEventListener listener = new XAXEventListener(event);
                    listener.addListener();
                    xaxEventListeners.add(listener);
                }
            }
        } finally {
            lock.unlock();
    ...
    void deactivateListener() {
        ...
        try {
            ...
            //
            xaxEventListeners.forEach(XAXEventListener::removeListener);
        } finally {
            lock.unlock();
        }
     ...
     * XAX event listener
     */
    private class XAXEventListener {

        /** Event handler */
        private final XAXEventHandler eventHandler;

        /**
         * Create the XAX event listener
         *
         * @param   eventRegistration           Event registration
         * @throws  EventListenerException      Invalid event
         */
        public XAXEventListener(EventRegistration eventRegistration) throws EventListenerException {
        ...
        /**
         * Add the XAX listener for this event
         */
        ...
        /**
         * Remove the XAX listener for this event
         */
        ...
        /**
         * Return the hash code for this XAX event listener
         *
         * @return                  Hash code
         */
        @Override
        ...
        /**
         * Check if two XAX events listeners are equal
         *
        ...
        public boolean equals(Object obj) {
            return (obj != null && (obj instanceof XAXEventListener) &&
                    eventHandler.equals(((XAXEventListener)obj).eventHandler));
        }

        /**
         * XAX listener event handler
         */
        private abstract class XAXEventHandler {
            ...
            /**
             * Create the XAX event handler
             *
             * @param   eventRegistration   Event registration
             */
            public XAXEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public boolean equals(Object obj) {
                return (obj != null && (obj instanceof XAXEventHandler) &&
                                        owner == ((XAXEventHandler)obj).owner &&
                                        accountId == ((XAXEventHandler)obj).accountId &&
                                        event == ((XAXEventHandler)obj).event);
            }
        }
         ...
         * Peer event handler
         */
        private class PeerEventHandler extends XAXEventHandler implements Listener<Peer> {
         ...
         */
        private class BlockEventHandler extends XAXEventHandler implements Listener<Block> {
         ...
         */
        private class TransactionEventHandler extends XAXEventHandler implements Listener<List<? extends Transaction>> {
         ...
         */
        private class LedgerEventHandler extends XAXEventHandler implements Listener<LedgerEntry> {
    ...
    static class EventRegistration {
         ...
         *
         * @param   event           XAX listener event
        ...
        /**
         * Return the XAX listener event
         *
         * @return                  XAX listener event
         */
        public Enum<? extends Enum> getEvent() {
            return event;
        }
