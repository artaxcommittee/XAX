...
public class APIProxy {
    ...
    static final boolean enableAPIProxy = Constants.isLightClient ||
            (XAX.getBooleanProperty("xax.enableAPIProxy") && API.openAPIPort == 0 && API.openAPISSLPort == 0);
    private static final int blacklistingPeriod = XAX.getIntProperty("xax.apiProxyBlacklistingPeriod") / 1000;
    static final String forcedServerURL = XAX.getStringProperty("xax.forceAPIProxyServerURL", "");
    ...
    static boolean isActivated() {
        return Constants.isLightClient || (enableAPIProxy && XAX.getBlockchainProcessor().isDownloading());
    }

    void blacklistHost(String host) {
        blacklistedPeers.put(host, XAX.getEpochTime() + blacklistingPeriod);
