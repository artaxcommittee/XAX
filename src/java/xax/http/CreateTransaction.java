...
import static xax.http.JSONResponses.FEATURE_NOT_AVAILABLE;
import static xax.http.JSONResponses.INCORRECT_DEADLINE;
import static xax.http.JSONResponses.INCORRECT_EC_BLOCK;
import static xax.http.JSONResponses.INCORRECT_LINKED_FULL_HASH;
import static xax.http.JSONResponses.INCORRECT_WHITELIST;
import static xax.http.JSONResponses.MISSING_DEADLINE;
import static xax.http.JSONResponses.MISSING_SECRET_PHRASE;
import static xax.http.JSONResponses.NOT_ENOUGH_FUNDS;
...
abstract class CreateTransaction extends APIServlet.APIRequestHandler {
    ...
    private Appendix.Phasing parsePhasing(HttpServletRequest req) throws ParameterException {
        int finishHeight = ParameterParser.getInt(req, "phasingFinishHeight",
                XAX.getBlockchain().getHeight() + 1,
                XAX.getBlockchain().getHeight() + Constants.MAX_PHASING_DURATION + 1,
                true);
    ...
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId,
                                            long amountNQT, Attachment attachment) throws XAXException {
        ...
        if (ecBlockId != 0 && ecBlockId != XAX.getBlockchain().getBlockIdAtHeight(ecBlockHeight)) {
            return INCORRECT_EC_BLOCK;
        }
        if (ecBlockId == 0 && ecBlockHeight > 0) {
            ecBlockId = XAX.getBlockchain().getBlockIdAtHeight(ecBlockHeight);
        }
        ...
        try {
            Transaction.Builder builder = XAX.newTransactionBuilder(publicKey, amountNQT, feeNQT,
            ...
            } catch (XAXException.NotYetEncryptedException ignore) {}
        ...
        } catch (XAXException.NotYetEnabledException e) {
