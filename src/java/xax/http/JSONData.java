...
import xax.AccountRestrictions;
import xax.Alias;
import xax.Appendix;
import xax.Asset;
import xax.AssetDelete;
import xax.AssetDividend;
import xax.AssetTransfer;
import xax.Attachment;
import xax.Block;
import xax.Constants;
import xax.Currency;
import xax.CurrencyExchangeOffer;
import xax.CurrencyFounder;
import xax.CurrencyTransfer;
import xax.CurrencyType;
import xax.DigitalGoodsStore;
import xax.Exchange;
import xax.ExchangeRequest;
import xax.FundingMonitor;
import xax.Generator;
import xax.HoldingType;
import xax.MonetarySystem;
import xax.XAX;
import xax.Order;
import xax.PhasingPoll;
import xax.PhasingVote;
import xax.Poll;
import xax.PrunableMessage;
import xax.Shuffler;
import xax.Shuffling;
import xax.ShufflingParticipant;
...
public final class JSONData {
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance) {
        return accountBalance(account, includeEffectiveBalance, XAX.getBlockchain().getHeight());
    }
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance, int height) {
        ...
        if (account == null) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceXAX", "0");
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceXAX", account.getEffectiveBalanceXAX(height));
    ...
    static JSONObject lessor(Account account, boolean includeEffectiveBalance) {
        ...
        if (accountLease.getCurrentLesseeId() != 0) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceXAX", String.valueOf(account.getGuaranteedBalanceNQT() / Constants.ONE_XAX));
            }
        }
    ...
    static JSONObject shuffling(Shuffling shuffling, boolean includeHoldingInfo) {
        ...
        if (includeHoldingInfo && shuffling.getHoldingType() != HoldingType.XAX) {
        ...
        if (includeExecutedPhased) {
            ...
            try (DbIterator<PhasingPoll.PhasingPollResult> phasingPollResults = PhasingPoll.getApproved(block.getHeight())) {
                for (PhasingPoll.PhasingPollResult phasingPollResult : phasingPollResults) {
                    ...
                    if (includeTransactions) {
                        phasedTransactions.add(transaction(XAX.getBlockchain().getTransaction(phasedTransactionId)));
                    } else {
    ...
    static JSONObject pollResults(Poll poll, List<Poll.OptionResult> results, VoteWeighting voteWeighting) {
        ...
        if (voteWeighting.getMinBalanceModel() == VoteWeighting.MinBalanceModel.ASSET) {
            ...
            if (currency != null) {
                ...
                Transaction currencyIssuance = XAX.getBlockchain().getTransaction(voteWeighting.getHoldingId());
    ...
    static JSONObject transaction(Transaction transaction, Filter<Appendix> filter) {
        ...
        json.put("confirmations", XAX.getBlockchain().getHeight() - transaction.getHeight());
    ...
    private static void putExpectedTransaction(JSONObject json, Transaction transaction) {
        json.put("height", XAX.getBlockchain().getHeight() + 1);
        json.put("phased", transaction.getPhasing() != null);
    ...
    static void ledgerEntry(JSONObject json, LedgerEntry entry, boolean includeTransactions, boolean includeHoldingInfo) {
        ...
        if (includeTransactions && entry.getEvent().isTransaction()) {
            Transaction transaction = XAX.getBlockchain().getTransaction(entry.getEventId());
