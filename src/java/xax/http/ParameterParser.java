...
import xax.Currency;
import xax.CurrencyBuyOffer;
import xax.CurrencySellOffer;
...
import static xax.http.JSONResponses.*;
...
public final class ParameterParser {
    ...
    public static int getNumberOfConfirmations(HttpServletRequest req) throws ParameterException {
        return getInt(req, "numberOfConfirmations", 0, XAX.getBlockchain().getHeight(), false);
    }
    ...
    public static long getHoldingId(HttpServletRequest req, HoldingType holdingType) throws ParameterException {
        long holdingId = ParameterParser.getUnsignedLong(req, "holding", holdingType != HoldingType.XAX);
        if (holdingType == HoldingType.XAX && holdingId != 0) {
            throw new ParameterException(JSONResponses.incorrect("holding", "holding id should not be specified if holdingType is XAX"));
        }
        return holdingId;
    }
    ...
    public static Transaction.Builder parseTransaction(String transactionJSON, String transactionBytes, String prunableAttachmentJSON) throws ParameterException {
        ...
        if (transactionJSON != null) {
            try {
                ...
                return XAX.newTransactionBuilder(json);
            } catch (XAXException.ValidationException | RuntimeException | ParseException e) {
            ...
            try {
                ...
                return XAX.newTransactionBuilder(bytes, prunableAttachments);
            } catch (XAXException.ValidationException|RuntimeException | ParseException e) {
                Logger.logDebugMessage(e.getMessage(), e);
                JSONObject response = new JSONObject();
    ...
    public static Attachment.TaggedDataUpload getTaggedData(HttpServletRequest req) throws ParameterException, XAXException.NotValidException {
