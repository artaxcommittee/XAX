...
public final class PopOff extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        try {
            XAX.getBlockchainProcessor().setGetMoreBlocks(false);
            if (numBlocks > 0) {
                blocks = XAX.getBlockchainProcessor().popOffTo(XAX.getBlockchain().getHeight() - numBlocks);
            } else if (height > 0) {
                blocks = XAX.getBlockchainProcessor().popOffTo(height);
            } else {
                return JSONResponses.missing("numBlocks", "height");
            }
        } finally {
            XAX.getBlockchainProcessor().setGetMoreBlocks(true);
        }
        ...
        if (keepTransactions) {
            blocks.forEach(block -> XAX.getTransactionProcessor().processLater(block.getTransactions()));
        }
        return response;
    }
