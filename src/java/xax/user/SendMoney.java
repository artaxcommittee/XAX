...
import static xax.user.JSONResponses.NOTIFY_OF_ACCEPTED_TRANSACTION;
...
public final class SendMoney extends UserServlet.UserRequestHandler {
    ...
    @Override
    JSONStreamAware processRequest(HttpServletRequest req, User user) throws XAXException.ValidationException, IOException {
        if (user.getSecretPhrase() == null) {
            return null;
        }

        String recipientValue = req.getParameter("recipient");
        String amountValue = req.getParameter("amountXAX");
        String feeValue = req.getParameter("feeXAX");
        ...
        try {
            ...
            amountNQT = Convert.parseXAX(amountValue.trim());
            feeNQT = Convert.parseXAX(feeValue.trim());
            ...
            response.put("amountXAX", amountValue);
            response.put("feeXAX", feeValue);
        ...
        } else if (feeNQT < Constants.ONE_XAX || feeNQT > Constants.MAX_BALANCE_NQT) {

            JSONObject response = new JSONObject();
            response.put("response", "notifyOfIncorrectTransaction");
            response.put("message", "\"Fee\" must be at least 1 XAX!");
            response.put("recipient", recipientValue);
            response.put("amountXAX", amountValue);
            response.put("feeXAX", feeValue);
            response.put("deadline", deadlineValue);
        ...
        if (account == null || Math.addExact(amountNQT, feeNQT) > account.getUnconfirmedBalanceNQT()) {
            ...
            final Transaction transaction = XAX.newTransactionBuilder(user.getPublicKey(),
